...
In order to let each worker use its own gradient, I use ```model.no_sync()``` in the [code](https://github.com/juntang-zhuang/GSAM/blob/7970892956b9159fe5eb1f304f9f956d5b6a8d5f/gsam/gsam.py#L129), perform the gradient decomposition in GSAM for each worker separately, then synchronize the final $\nabla f^{GSAM}$ [here](https://github.com/juntang-zhuang/GSAM/blob/7970892956b9159fe5eb1f304f9f956d5b6a8d5f/gsam/gsam.py#L87). However, I'm not sure if ```model.no_sync()``` only works in ```DistributedDataParallel``` mode rather than ```DataParallel``` mode. 
